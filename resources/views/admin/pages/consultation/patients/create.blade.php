        ...
        <section class="container-fluid">
            ...
            <div class="card mb-4 h-100">
                <div class="card-header justify-content-between align-items-center d-flex">
                    <h6 class="card-title m-0">Create Patient</h6>
                    {{-- <a href="#" class="btn btn-outline-secondary btn-sm text-body"><i
                            class="ri-download-2-line align-middle"></i> Export</a> --}}
                </div>
                <form action="{{ route('dashboard.consultation.patients.store') }}" method="post">
                    @csrf
                    <div class="card-body">
                                    ...
                                    @if ($key == 'sex')
                                        <select name="{{ $key }}" class="form-control"
                                            ...
                                            @foreach ($sex_options as $option)
                                                <option value="{{ $option }}"
                                                    {{ old($key) == $option ? 'selected' : '' }}>{{ $option }}
                                                </option>
                                            @endforeach
                                        </select>
                                    @else
                                        <input name="{{ $key }}" class="form-control" type="{{$field["type"] ?? "text"}}"
                                            {{ $field['required'] ?? false ? 'required' : '' }}
                                            value="{{ old($key) }}" />
                                    @endif
                                    @error($key)
